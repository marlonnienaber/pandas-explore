...
# TODO: proof against float special case
def explore_int_id(column):
    """
    Explores a column of a pandas dataframe containing integer primary or foreign keys:
    1) missing/faulty values
    - displays whether column contains only entries of type int
    - if present displays non-int values and how often they occur
    2) duplicates 
    - displays whether valid entries contain duplicates
    - if duplicates are present displays which values occur multiple times
    ...
    contains_non_ints = any(column.apply(lambda x: not isinstance(x, int)))
    if not contains_non_ints:
        messages.append("contains only valid ints")
        
    int_entries = column[column.apply(lambda x: isinstance(x, int))]
    ints_contain_duplicates = len(int_entries.unique()) != len(int_entries)
    if not ints_contain_duplicates and len(int_entries) > 0:
        messages.append("valid entries contain no duplicates")
    
    
    if contains_non_ints:
        non_int_entries = column[column.apply(lambda x: not isinstance(x, int))]
        chart_html = create_bicategorical_pie_chart(len(column),len(non_int_entries),"int entries", "non-int entries")
        html_list.append(chart_html)
        non_int_entries_with_types = non_int_entries.apply(lambda x: f"{x} ({type(x).__name__})")
        non_int_counts = non_int_entries_with_types.value_counts(dropna = False)
        non_int_df = pd.DataFrame(non_int_counts).reset_index()
        non_int_df.columns = ['non-int entries', 'count']
        non_int_html = non_int_df.head(15).to_html(index=False)
        if len(non_int_df) > 15:
            truncated_html = "<p style='color: grey;'>(truncated to 15 rows)</p>"
            combined_html = f"{non_int_html}{truncated_html}"
        else:
            combined_html = non_int_html
        html_list.append(combined_html)
        
    if ints_contain_duplicates:
        duplicated_values = int_entries[int_entries.duplicated(keep=False)].unique()
...
def explore_string_id(column):
    ...
    if contains_non_strings:
        ...
        create_bicategorical_pie_chart(len(column),len(non_string_entries))
...
def explore_boolean_attribute(column):
    ...
    if contains_non_bools:
        ...
        create_bicategorical_pie_chart(len(column),len(non_bool_entries))
...
def explore_categorical_attribute(column):
    ...
    display_in_grid(None, [chart_html])
